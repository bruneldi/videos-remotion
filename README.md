...
Este export en .zip **no incluye** las Remotion Agent Skills oficiales
(`.agents/skills` y los symlinks de `.claude/skills`), ya que dependen de
symlinks que no siempre se conservan bien al comprimir/descomprimir. Para
reinstalarlas (asistencia de Claude Code con patrones específicos de
Remotion: creación, captions, render, markup, etc.), corré en la raíz del
proyecto:
...
Esto crea `.agents/skills` con el contenido de cada skill y `.claude/skills`
con symlinks hacia ellas (11 skills en total: `remotion-best-practices`,
`remotion-captions`, `remotion-create`, `remotion-docs`,
`remotion-interactivity`, `remotion-maps`, `remotion-markup`,
`remotion-multimedia`, `remotion-render`, `remotion-saas`,
`remotion-upgrade`). Para actualizarlas más adelante, `npx skills update`.
